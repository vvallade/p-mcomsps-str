...
\author{
   \IEEEauthorblockN{
      ...
      Souheib Baarir\IEEEauthorrefmark{1}\IEEEauthorrefmark{2},
      Ludovic Le Frioux\IEEEauthorrefmark{3},
   ...
   \IEEEauthorblockA{
      \IEEEauthorrefmark{2}Universit\'e Paris Nanterre, France
   }
   \IEEEauthorblockA{
      \IEEEauthorrefmark{3}LRDE, EPITA, Le Kremlin-Bic\^ectre, France
   }
   \IEEEauthorblockA{
...
import clauses; (3) we added an option to use or not the GE preprocessing.
...
where the underlying core engines are either \texttt{L} or \texttt{V}
instances.
...
used by~\cite{hordesat_15}. We instantiate a \texttt{Sharer} per solver (the
producer). It gets clauses from this producer and exports some of them to all
others (the consumers).

The exchange strategy is  defined  as follows: each solver exports clauses
having a LBD value under a given threshold (2 at the beginning). Every 0.5
...
concerned solver is increased if an insufficient number of literals (less than
1200) are dispatched.
...
\ludovic{The whole section should be written!}
